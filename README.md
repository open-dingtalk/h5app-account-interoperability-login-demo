# 代码模板——账号打通后免登登陆企业OA系统

## 1. 背景介绍

当企业OA或三方系统需要集成钉钉，可以通过接入钉钉免登能力，获取钉钉用户账号的基本信息，使用手机号或邮箱作为账号互通标识登陆企业内部系统。也可以通过不同系统之间账号映射关系匹配方案，提前准备好账号的映射关系，通过钉钉账号关联映射关系后登陆企业内部系统。

## 2. 项目结构

> **fronted**：前端模块，使用vue构建，主要功能有：接入钉钉JSAPI获取authCode、录入用户信息。
>
> **backend**：后端模块，使用springboot构建，主要功能有：使用authCode获取access_token、使用access_token获取用户信息等。
...
│           │               └── service
│           │                   └── DingTalkLoginServiceImpl.java     ----- 核心业务代码
│           └── resources
...
```shell
git clone https://github.com/open-dingtalk/h5app-account-interoperability-login-demo.git
```
...
输入信息，自动绑定当前钉钉账号 
![](https://img.alicdn.com/imgextra/i1/O1CN01NRAIJV21f5YBBTwmG_!!6000000007011-2-tps-1065-415.png)


## 6. 参考文档

1. 获取微应用免登授权码，文档链接：https://open.dingtalk.com/document/orgapp-client/obtain-the-micro-application-logon-free-authorization-code
2. 获取企业内部应用access_token，文档链接：https://open.dingtalk.com/document/orgapp-server/obtain-orgapp-token
3. 获取用户信息，文档链接：https://open.dingtalk.com/document/orgapp-server/obtain-the-userid-of-a-user-by-using-the-log-free
